...
class FeetechMotorsBus(SerialMotorsBus):
    ...
    def _assert_same_protocol(self) -> None:
        # The protocol actually used by a physical Feetech servo is a property of the hardware/firmware
        # (e.g. some STS3215 batches speak Protocol 1), and it cannot be changed through any register.
        # `MODEL_PROTOCOL` only encodes the protocol LeRobot expects by default. When the user explicitly
        # passes a different `protocol_version`, trust them and downgrade the mismatch to a warning.
        mismatched = [model for model in self.models if MODEL_PROTOCOL[model] != self.protocol_version]
        if mismatched:
            logger.warning(
                f"protocol_version={self.protocol_version} differs from the protocol expected for "
                f"{mismatched} ({[MODEL_PROTOCOL[m] for m in mismatched]}). "
                "Assuming the motors on this bus run the requested protocol."
            )

    def _assert_protocol_is_compatible(self, instruction_name: str) -> None:
        # 'sync_read' is available for Protocol 1 via a sequential-read fallback (see `_sync_read`).
    ...
    def _sync_read(
        ...
        *,
        ...
        """Protocol 1 has no group 'Sync Read' instruction, so read each motor sequentially.

        Falls back to the group read (protocol 0) otherwise.
        """
        ...
        for id_ in motor_ids:
            value, comm, error = self._read(
                addr, length, id_, num_retry=num_retry, raise_on_error=raise_on_error, err_msg=err_msg
            )
    ...
    def _read(
        ...
        *,
        ...
        # The scservo_sdk assembles multi-byte registers big-endian in Protocol-1 mode, but the
        # STS3215 stores them little-endian. Reassemble so the raw value matches the servo's byte
        # order. Writes are unaffected because they are already serialized LSB-first.
        value, comm, error = super()._read(
            address,
            length,
            motor_id,
            num_retry=num_retry,
    ...
    def ping(self, motor: NameOrID, num_retry: int = 0, raise_on_error: bool = False) -> int | None:
        ...
        The `scservo_sdk.ping()` reads the model number at address 3-4 and assembles it big-endian,
        which is only correct for Protocol-0 feetech servos. For Protocol 1 the model number must be
        read little-endian (see `_read`), so ping for presence with the SDK and re-read the model
        number through the byte-order-aware `_read`.
        """
        if self.protocol_version != 1:
            return super().ping(motor, num_retry=num_retry, raise_on_error=raise_on_error)
        ...
        for n_try in range(1 + num_retry):
            # Presence ping; the SDK also reads the (wrong) model at address 3-4, but we only rely on
            # its communication result here.
            _, comm, error = self.packet_handler.ping(self.port_handler, id_)
            if self._is_comm_success(comm) and not self._is_error(error):
                model_number, comm, error = self._read(*MODEL_NUMBER, id_, raise_on_error=False)
                if self._is_comm_success(comm) and not self._is_error(error):
                    break
    ...
    def write_calibration(self, calibration_dict: dict[str, MotorCalibration], cache: bool = True) -> None:
        for motor, calibration in calibration_dict.items():
            # The official code only writes Homing_Offset for protocol 0, because the SCS series
            # (protocol 1) it was written against lacks that register. Our STS3215 servos run protocol 1
            # and DO use Homing_Offset (Present_Position = Actual_Position - Homing_Offset), so it must be
            # written regardless of protocol version.
    ...
    def _split_into_byte_chunks(self, value: int, length: int) -> list[int]:
        # Always emit little-endian bytes (LSB first). The physical STS/SMS servos store multi-byte
        # registers little-endian regardless of the bus protocol; the scservo_sdk's `SCS_LOBYTE` /
        # `SCS_HIBYTE` macros are endian-dependent (they swap for Protocol 1), which would corrupt writes.
        if length == 1:
            data = [value]
        elif length == 2:
